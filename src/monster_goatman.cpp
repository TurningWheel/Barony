	...
	if ( !MONSTER_INIT )
	{
		if ( myStats != nullptr )
		{
				...
				&& myStats->leader_uid == 0 )
			{
				...
				//Spawn in potions.
				int end = map_rng.rand()%NUM_GOATMAN_BOSS_GHARBAD_POTIONS + 5;
				for ( int i = 0; i < end; ++i )
				{
					switch ( map_rng.rand()%10 )
					{
						...
						case 8:
							...
							break;
						case 9:
							newItem(POTION_HEALING, EXCELLENT, 0, 1, map_rng.rand(), false, &myStats->inventory);
							break;
						default:
				...
				newItem(CRYSTAL_SHURIKEN, EXCELLENT, 1 + map_rng.rand()%1, map_rng.rand()%NUM_GOATMAN_BOSS_GHARBAD_THROWN_WEAPONS + 2, map_rng.rand(), true, &myStats->inventory);
			}

			// random effects
			if ( map_rng.rand() % 8 == 0 )
			{
				my->setEffect(EFF_ASLEEP, true, 1800 + map_rng.rand() % 1800, false);
			}
			...
			bool isShaman = false;
			if ( map_rng.rand() % 2 && boss == 0 && !minion )
			{
				isShaman = true;
				if ( myStats->leader_uid == 0 && !my->flags[USERFLAG2] && map_rng.rand() % 2 == 0 )
				{
			...
			switch ( defaultItems )
			{
				...
				case 2:
				case 1:
					if ( isShaman && map_rng.rand() % 10 == 0 )
					{
						switch ( map_rng.rand() % 4 )
						{
							case 0:
								newItem(SPELLBOOK_SLOW, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, &myStats->inventory);
								break;
							case 1:
								newItem(SPELLBOOK_FIREBALL, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, &myStats->inventory);
								break;
							case 2:
								newItem(SPELLBOOK_COLD, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, &myStats->inventory);
								break;
							case 3:
								newItem(SPELLBOOK_FORCEBOLT, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, &myStats->inventory);
								break;
						}
					}
					break;
			...
			if ( !boss )
			{
				if ( !isShaman && map_rng.rand() % 3 > 0 )
				{
					newItem(STEEL_CHAKRAM, SERVICABLE, 0, map_rng.rand()%NUM_GOATMAN_THROWN_WEAPONS + 1, map_rng.rand(), false, &myStats->inventory);
				}
				int numpotions = map_rng.rand() % NUM_GOATMAN_POTIONS + 2;
				if ( map_rng.rand() % 3 == 0 )
				{
					int numhealpotion = map_rng.rand() % 2 + 1;
					newItem(POTION_HEALING, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), 0, numhealpotion, map_rng.rand(), false, &myStats->inventory);
					numpotions -= numhealpotion;
				}
				if ( map_rng.rand() % 4 > 0 )
				{
					// undroppable
					newItem(POTION_BOOZE, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), 0, numpotions, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, &myStats->inventory);
				}
			}
			...
			if ( isShaman )
			{
				...
				if ( myStats->shield == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_SHIELD] == 1 )
				{
					...
					switch ( map_rng.rand() % 20 )
					{
						...
						case 6:
						case 7:
							myStats->shield = newItem(TOOL_CRYSTALSHARD, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						case 8:
							myStats->shield = newItem(MIRROR_SHIELD, static_cast<Status>(map_rng.rand() % 4 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						default:
							myStats->shield = newItem(TOOL_LANTERN, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
					}
				}
				// give cloak
				...
				if ( myStats->helmet == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_HELM] == 1 )
				{
					switch ( map_rng.rand() % 10 )
					{
						case 0:
						case 1:
							myStats->helmet = newItem(HAT_HOOD, WORN, -1 + map_rng.rand() % 3, 1, 0, false, nullptr);
							break;
						default:
							myStats->helmet = newItem(HAT_WIZARD, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
					}
				}
				// give armor
				if ( myStats->breastplate == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_ARMOR] == 1 )
				{
					switch ( map_rng.rand() % 10 )
					{
						case 0:
						case 1:
							myStats->breastplate = newItem(WIZARD_DOUBLET, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						case 2:
							myStats->breastplate = newItem(LEATHER_BREASTPIECE, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						default:
							break;
					}
				...
				if ( myStats->shoes == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_BOOTS] == 1 )
				{
					switch ( map_rng.rand() % 20 )
					{
						...
						case 2:
						case 3:
							myStats->shoes = newItem(IRON_BOOTS, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						case 19:
							myStats->shoes = newItem(CRYSTAL_BOOTS, static_cast<Status>(map_rng.rand() % 4 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						default:
							myStats->shoes = newItem(STEEL_BOOTS, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
					}
				}
				// give weapon
				if ( myStats->weapon == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_WEAPON] == 1 )
				{
					switch ( map_rng.rand() % 12 )
					{
						...
						case 3:
						case 4:
							myStats->weapon = newItem(MAGICSTAFF_COLD, static_cast<Status>(map_rng.rand() % 2 + SERVICABLE), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						case 5:
						case 6:
						case 7:
						case 8:
							myStats->weapon = newItem(MAGICSTAFF_FIRE, static_cast<Status>(map_rng.rand() % 2 + SERVICABLE), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						case 9:
							switch ( map_rng.rand() % 4 )
							{
								case 0:
									myStats->weapon = newItem(SPELLBOOK_SLOW, static_cast<Status>(map_rng.rand() % 2 + DECREPIT), -1 + map_rng.rand() % 3, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, nullptr);
									break;
								case 1:
									myStats->weapon = newItem(SPELLBOOK_FIREBALL, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, nullptr);
									break;
								case 2:
									myStats->weapon = newItem(SPELLBOOK_COLD, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, nullptr);
									break;
								case 3:
									myStats->weapon = newItem(SPELLBOOK_FORCEBOLT, static_cast<Status>(map_rng.rand() % 3 + DECREPIT), -1 + map_rng.rand() % 3, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, nullptr);
									break;
							}
							break;
						default:
							myStats->weapon = newItem(MAGICSTAFF_SLOW, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
					}
				}
			}
			else
			{
				...
				//		break;
				//	default:
				//		myStats->helmet = newItem(HAT_WIZARD, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, 0, false, nullptr);
				//		break;
				//	}
				//}
				// give armor
				if ( myStats->breastplate == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_ARMOR] == 1 )
				{
					...
					case 1:
					case 2:
						myStats->breastplate = newItem(STEEL_BREASTPIECE, static_cast<Status>(map_rng.rand() % 4 + DECREPIT), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					case 3:
					case 4:
						myStats->breastplate = newItem(LEATHER_BREASTPIECE, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					case 5:
					case 6:
						myStats->breastplate = newItem(IRON_BREASTPIECE, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					case 19:
						myStats->breastplate = newItem(CRYSTAL_BREASTPIECE, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					default:
						break;
					}
				...
				if ( myStats->weapon == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_WEAPON] == 1 )
				{
					...
					case 6:
					case 7:
						myStats->weapon = newItem(STEEL_AXE, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					case 18:
						myStats->weapon = newItem(CRYSTAL_BATTLEAXE, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					case 19:
						myStats->weapon = newItem(CRYSTAL_MACE, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					default:
						myStats->weapon = newItem(STEEL_MACE, static_cast<Status>(map_rng.rand() % 3 + WORN), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					}
				}
			}
...
void goatmanDie(Entity* my)
{
	...
	playSoundEntity(my, 338 + map_rng.rand() % 2, 128);
...
void Entity::goatmanChooseWeapon(const Entity* target, double dist)
{
	...
	if ( !myStats->weapon || isMeleeWeapon(*myStats->weapon) )
	{
		...
		if ( monsterSpecialTimer == 0 && (ticks % 10 == 0) && monsterAttack == 0 && map_rng.rand() % 10 == 0 )
		{
