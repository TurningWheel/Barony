...
void updateCharacterSheet()
{
    ...
    ttfPrintTextFormatted(ttf12, 8, 250, language[2499], stats[clientnum]->HP, stats[clientnum]->MAXHP, stats[clientnum]->MP, stats[clientnum]->MAXMP);
	...
	printStatBonus(ttf12, stats[clientnum]->STR, statGetSTR(stats[clientnum]), 8 + longestline(statText) * TTF12_WIDTH, 274);

	snprintf(statText, 64, language[1201], stats[clientnum]->DEX);
	ttfPrintTextFormatted(ttf12, 8, 286, statText);
	printStatBonus(ttf12, stats[clientnum]->DEX, statGetDEX(stats[clientnum]), 8 + longestline(statText) * TTF12_WIDTH, 286);

	snprintf(statText, 64, language[1202], stats[clientnum]->CON);
	ttfPrintTextFormatted(ttf12, 8, 298, statText);
	printStatBonus(ttf12, stats[clientnum]->CON, statGetCON(stats[clientnum]), 8 + longestline(statText) * TTF12_WIDTH, 298);

	snprintf(statText, 64, language[1203], stats[clientnum]->INT);
	ttfPrintTextFormatted(ttf12, 8, 310, statText);
	printStatBonus(ttf12, stats[clientnum]->INT, statGetINT(stats[clientnum]), 8 + longestline(statText) * TTF12_WIDTH, 310);

	snprintf(statText, 64, language[1204], stats[clientnum]->PER);
	ttfPrintTextFormatted(ttf12, 8, 322, statText);
	printStatBonus(ttf12, stats[clientnum]->PER, statGetPER(stats[clientnum]), 8 + longestline(statText) * TTF12_WIDTH, 322);

	snprintf(statText, 64, language[1205], stats[clientnum]->CHR);
	ttfPrintTextFormatted(ttf12, 8, 334, statText);
	printStatBonus(ttf12, stats[clientnum]->CHR, statGetCHR(stats[clientnum]), 8 + longestline(statText) * TTF12_WIDTH, 334);

	// armor, gold, and weight
	ttfPrintTextFormatted(ttf12, 8, 358, language[370], stats[clientnum]->GOLD);
	ttfPrintTextFormatted(ttf12, 8, 370, language[371], AC(stats[clientnum]));
	Uint32 weight = 0;
