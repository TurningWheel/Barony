...
void updateRemoveCurseGUI() {
	...
	if (removecursegui_active) {
		...
		if (!removecurse_inventory) {
			...
			if (buttonclick == 7) {
				pos.x = REMOVECURSE_GUI_X + (identifyGUI_img->w - 28); pos.y = REMOVECURSE_GUI_Y + 16;
				pos.w = 0; pos.h = 0;
				drawImage(invup_bmp, NULL, &pos);
			}
			//Remove Curse GUI down button.
			if (buttonclick == 8) {
				pos.x = REMOVECURSE_GUI_X + (identifyGUI_img->w - 28); pos.y = REMOVECURSE_GUI_Y + 52;
				pos.w = 0; pos.h = 0;
				drawImage(invdown_bmp, NULL, &pos);
			}
			//Remove Curse GUI close button.
			if (buttonclick == 9) {
				pos.x = REMOVECURSE_GUI_X + 393; pos.y = REMOVECURSE_GUI_Y;
				pos.w = 0; pos.h = 0;
