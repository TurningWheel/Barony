...
void GenericGUIMenu::alchemyCombinePotions()
{
	...
	{
		// do duplicate.
		if ( local_rng.getU32() % 100 < (50 + skillLVL * 10) ) // 50 - 100% chance
		{
	...
	else
	{
		...
		if ( local_rng.getU32() % 100 < (50 + skillLVL * 5) ) // 50 - 75% chance
		{
			emptyBottle = true;
		}
	}
	...
	for ( auto it = potionStandardAppearanceMap.begin(); it != potionStandardAppearanceMap.end(); ++it )
	{
		if ( (*it).first == result )
		{
			...
			if ( result == POTION_SICKNESS )
			{
				if ( !samePotion )
				{
					appearance = 0 + local_rng.getU32() % items[POTION_SICKNESS].variations;
				}
			...
			if ( raiseSkill && local_rng.getU32() % 2 == 0 )
			{
				if ( multiplayer == CLIENT )
				{
					// request level up
	...
	for ( auto it = potionStandardAppearanceMap.begin(); it != potionStandardAppearanceMap.end(); ++it )
	{
		...
		if ( (*it).first == type )
		{
			...
			else
			{
				...
				gameStatistics[STATISTICS_ALCHEMY_RECIPES] |= (1 << index); 
				if ( increaseskill && local_rng.getU32() % 6 == 0 )
				{
					if ( multiplayer == CLIENT )
					{
						// request level up
...
bool GenericGUIMenu::tinkeringSalvageItem(Item* item, bool outsideInventory, int player)
{
	...
	if ( stats[player] && players[player] && item->status > BROKEN )
	{
		...
		{
			case 5:
				bonusMetalScrap = (1 + ((local_rng.getU32() % 2 == 0) ? 1 : 0)) * metal; // 2x or 50% 3x extra scrap
				bonusMagicScrap = (1 + ((local_rng.getU32() % 2 == 0) ? 1 : 0)) * magic; // 2x or 50% 3x extra scrap
				break;
			case 4:
				bonusMetalScrap = (1 + ((local_rng.getU32() % 4 == 0) ? 1 : 0)) * metal; // 2x or 25% 3x extra scrap
				bonusMagicScrap = (1 + ((local_rng.getU32() % 4 == 0) ? 1 : 0)) * magic; // 2x or 25% 3x extra scrap
				break;
			case 3:
				bonusMetalScrap = ((local_rng.getU32() % 2 == 0) ? 1 : 0) * metal; // 50% 2x scrap value
				bonusMagicScrap = ((local_rng.getU32() % 2 == 0) ? 1 : 0) * magic; // 50% 2x scrap value
				break;
			case 2:
				bonusMetalScrap = ((local_rng.getU32() % 4 == 0) ? 1 : 0) * metal; // 25% 2x scrap value
				bonusMagicScrap = ((local_rng.getU32() % 4 == 0) ? 1 : 0) * magic; // 25% 2x scrap value
				break;
			case 1:
				bonusMetalScrap = ((local_rng.getU32() % 8 == 0) ? 1 : 0) * metal; // 12.5% 2x scrap value
				bonusMagicScrap = ((local_rng.getU32() % 8 == 0) ? 1 : 0) * magic; // 12.5% 2x scrap value
				break;
			default:
				break;
		}
	...
	if ( stats[player] && didCraft )
	{
		if ( metal >= 4 || magic >= 4 )
		{
			if ( local_rng.getU32() % 2 == 0 ) // 50%
			{
				if ( stats[player]->PROFICIENCIES[PRO_LOCKPICKING] < SKILL_LEVEL_EXPERT )
				{
					increaseSkill = true;
				}
				else if ( local_rng.getU32() % 20 == 0 && !tinkeringBulkSalvage )
				{
		...
		{
			if ( (ticks - tinkeringSfxLastTicks) > 200 && ((metal >= 4 || magic >= 4) || local_rng.getU32() % 5 == 0) )
			{
				tinkeringSfxLastTicks = ticks;
				playSoundEntity(players[player]->entity, 421 + (local_rng.getU32() % 2) * 3, 64);
			}
			else
			{
				if ( local_rng.getU32() % 4 == 0 )
				{
					playSoundEntity(players[player]->entity, 35 + local_rng.getU32() % 3, 64);
				}
				else
				{
					playSoundEntity(players[player]->entity, 462 + local_rng.getU32() % 2, 64);
				}
			}
		}
	}
...
Item* GenericGUIMenu::tinkeringCraftItemAndConsumeMaterials(const Item* item)
{
	...
	if ( tinkeringPlayerHasMaterialsInventory(metal, magic) )
	{
		...
		if ( (ticks - tinkeringSfxLastTicks) > 100 )
		{
			...
			{
				playSoundEntity(players[gui_player]->entity, 459 + (local_rng.getU32() % 3), 92);
			}
			else
			{
				if ( local_rng.getU32() % 3 == 0 )
				{
					playSoundEntity(players[gui_player]->entity, 422 + (local_rng.getU32() % 2), 92);
				}
				else
				{
					playSoundEntity(players[gui_player]->entity, 35 + local_rng.getU32() % 3, 64);
				}
			}
		}
		else
...
int GenericGUIMenu::scribingToolDegradeOnUse(Item* itemUsedWith)
{
	...
	if ( randomValue > 0 )
	{
		usageCost += local_rng.getU32() % (randomValue + 1);
	}
	...
	if ( itemCategory(item) == SCROLL )
	{
		...
		if ( crafted )
		{
			if ( crafted->type == SCROLL_MAIL )
			{
				...
				crafted->appearance = local_rng.getU32();
			}
			Item* pickedUp = itemPickup(gui_player, crafted);
