...
namespace ConsoleCommands {
	...
	static ConsoleCommand ccmd_spawnitem("/spawnitem", "spawn an item (cheat)", []CCMD{
		...
		for ( c = 0; c < NUMITEMS; c++ )
		{
			...
			{
				dropItem(newItem(static_cast<ItemType>(c), EXCELLENT, 0, 1, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
				break;
			}
		}
		if ( c == NUMITEMS )
	...
	static ConsoleCommand ccmd_spawncursed("/spawncursed", "spawn a cursed item (cheat)", []CCMD{
		...
		for ( c = 0; c < NUMITEMS; c++ )
		{
			if ( strcmp(items[c].name_identified, name.c_str()) == 0 )
			{
				dropItem(newItem(static_cast<ItemType>(c), WORN, -2, 1, local_rng.getU32(), false, &stats[clientnum]->inventory), 0);
				break;
			}
		}
		if ( c == NUMITEMS )
	...
	static ConsoleCommand ccmd_maxout2("/maxout2", "give player lots of stuff (cheat)", []CCMD{
		...
		if ( multiplayer != CLIENT )
		{
			...
			myStats->weapon = newItem(STEEL_SWORD, SERVICABLE, 0, 1, local_rng.getU32(), true, &myStats->inventory);
			newItem(CROSSBOW, SERVICABLE, 0, 1, local_rng.getU32(), true, &myStats->inventory);
			newItem(MAGICSTAFF_LIGHT, SERVICABLE, 0, 1, local_rng.getU32(), true, &myStats->inventory);
			myStats->shield = newItem(STEEL_SHIELD, SERVICABLE, 0, 1, local_rng.getU32(), true, &myStats->inventory);
			myStats->helmet = newItem(HAT_HOOD, SERVICABLE, 0, 1, 2, true, &myStats->inventory);
			myStats->shoes = newItem(STEEL_BOOTS, SERVICABLE, 0, 1, local_rng.getU32(), true, &myStats->inventory);
			myStats->breastplate = newItem(STEEL_BREASTPIECE, SERVICABLE, 0, 1, local_rng.getU32(), true, &myStats->inventory);
			myStats->gloves = newItem(GAUNTLETS, SERVICABLE, 0, 1, local_rng.getU32(), true, &myStats->inventory);
			myStats->cloak = newItem(CLOAK_BLACK, SERVICABLE, 0, 1, local_rng.getU32(), true, &myStats->inventory);
		}
		else
		{
			for ( int c = 0; c < 24; c++ )
	...
	static ConsoleCommand ccmd_splitscreen("/splitscreen", "enable splitscreen mode (cheat)", []CCMD{
		...
		for ( int i = 0; i < MAXPLAYERS; ++i )
		{
			...
			if ( i > 0 )
			{
				stats[i]->sex = static_cast<sex_t>(local_rng.getU32() % 2);
				stats[i]->appearance = local_rng.getU32() % 18;
				stats[i]->clearStats();
				client_classes[i] = local_rng.getU32() % (CLASS_MONK + 1);//NUMCLASSES;
				stats[i]->playerRace = RACE_HUMAN;
				if ( enabledDLCPack1 || enabledDLCPack2 )
				{
					stats[i]->playerRace = local_rng.getU32() % NUMPLAYABLERACES;
					if ( !enabledDLCPack1 )
					{
					...
					{
						client_classes[i] = local_rng.getU32() % (NUMCLASSES);
						if ( !enabledDLCPack1 )
						{
					...
					{
						client_classes[i] = local_rng.getU32() % (CLASS_MONK + 2);
				...
				strcpy(stats[i]->name, randomPlayerNamesFemale[local_rng.getU32() % randomPlayerNamesFemale.size()].c_str());
				bool oldIntro = intro;
	...
	static ConsoleCommand ccmd_gimmearrows("/gimmearrows", "give the player some arrows (cheat)", []CCMD{
		...
		{
			dropItem(newItem(static_cast<ItemType>(i), EXCELLENT, 0, 25 + local_rng.getU32() % 26, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		}
		});
	...
	static ConsoleCommand ccmd_gimmescrap("/gimmescrap", "give the player some scrap metal (cheat)", []CCMD{
		...
		dropItem(newItem(TOOL_METAL_SCRAP, EXCELLENT, 0, 100, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		dropItem(newItem(TOOL_MAGIC_SCRAP, EXCELLENT, 0, 100, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		dropItem(newItem(TOOL_TINKERING_KIT, EXCELLENT, 0, 1, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		});
	...
	static ConsoleCommand ccmd_gimmerobots("/gimmerobots", "give the player some robots (cheat)", []CCMD{
		...
		dropItem(newItem(TOOL_GYROBOT, EXCELLENT, 0, 10, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		dropItem(newItem(TOOL_DUMMYBOT, EXCELLENT, 0, 10, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		dropItem(newItem(TOOL_SENTRYBOT, EXCELLENT, 0, 10, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		dropItem(newItem(TOOL_SPELLBOT, EXCELLENT, 0, 10, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		});
	...
	static ConsoleCommand ccmd_gimmegoblinbooks("/gimmegoblinbooks", "give the player some spellbooks (cheat)", []CCMD{
		...
		for ( int i = 0; i < NUM_SPELLS; ++i )
		{
			...
			dropItem(newItem(static_cast<ItemType>(spellbook), DECREPIT, -1, 1, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		}
		});
	...
	static ConsoleCommand ccmd_gimmebombs("/gimmebombs", "give the player some bombs (cheat)", []CCMD{
		...
		dropItem(newItem(TOOL_BOMB, EXCELLENT, 0, 10, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		dropItem(newItem(TOOL_FREEZE_BOMB, EXCELLENT, 0, 10, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		dropItem(newItem(TOOL_TELEPORT_BOMB, EXCELLENT, 0, 10, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		dropItem(newItem(TOOL_SLEEP_BOMB, EXCELLENT, 0, 10, local_rng.getU32(), true, &stats[clientnum]->inventory), 0);
		});
	...
	static ConsoleCommand ccmd_gimmeblessedpotions("/gimmeblessedpotions", "give blessed potions (cheat)", []CCMD{
		...
		for ( int i = 0; i < potionStandardAppearanceMap.size(); ++i )
		{
			auto generatedPotion = potionStandardAppearanceMap.at(i);
			Item* potion = newItem(static_cast<ItemType>(generatedPotion.first), static_cast<Status>(SERVICABLE + local_rng.getU32() % 2),
				1 + local_rng.getU32() % 2, 1, generatedPotion.second, true, nullptr);
			itemPickup(clientnum, potion);
			//free(potion);
		}
		});
	...
	static ConsoleCommand ccmd_gimmecursedpotions("/gimmecursedpotions", "give cursed potions (cheat)", []CCMD{
		...
		for ( int i = 0; i < potionStandardAppearanceMap.size(); ++i )
		{
			auto generatedPotion = potionStandardAppearanceMap.at(i);
			Item* potion = newItem(static_cast<ItemType>(generatedPotion.first), static_cast<Status>(SERVICABLE + local_rng.getU32() % 2),
				-2 + local_rng.getU32() % 2, 1, generatedPotion.second, true, nullptr);
			itemPickup(clientnum, potion);
			//free(potion);
		}
		});
	...
	static ConsoleCommand ccmd_gimmexp("/gimmexp", "give the player some XP (cheat)", []CCMD{
		...
		if ( players[clientnum] && players[clientnum]->entity )
		{
			players[clientnum]->entity->getStats()->EXP += 1 + local_rng.getU32() % 50;
		}
		});
	...
	static ConsoleCommand ccmd_classdebug("/classdebug", "", []CCMD{
		...
		client_classes[clientnum] = local_rng.getU32() % (CLASS_MONK + 1);//NUMCLASSES;
		});
	...
	static void rocksFall(int player) {
		...
		entity->y = players[player]->entity->y;
		entity->z = -64;
		entity->yaw = (PI / 2) * (local_rng.getU32() % 4);
		entity->sizex = 7;
		entity->sizey = 7;
