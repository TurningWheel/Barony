		...
		if ( myStats != nullptr )
		{
			...
			switch ( defaultItems )
			{
				...
				case 2:
					...
					{
						int i = 1 + map_rng.rand() % 4;
						for ( c = 0; c < i; c++ )
						{
							newItem(static_cast<ItemType>(GEM_GARNET + map_rng.rand() % 15), static_cast<Status>(1 + map_rng.rand() % 4), 0, 1, map_rng.rand(), false, &myStats->inventory);
						}
					}
				case 1:
					if ( map_rng.rand() % 3 == 0 )
					{
						newItem(FOOD_FISH, EXCELLENT, 0, 1, map_rng.rand(), false, &myStats->inventory);
					}
					break;
				default:
					break;
			...
			if ( myStats->shield == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_SHIELD] == 1 )
			{
				switch ( map_rng.rand() % 10 )
				{
					...
					case 8:
					case 9:
						myStats->shield = newItem(WOODEN_SHIELD, static_cast<Status>(WORN + map_rng.rand() % 2), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
				}
			}
			...
			if ( myStats->weapon == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_WEAPON] == 1 )
			{
				switch ( map_rng.rand() % 10 )
				{
					...
					case 3:
					case 4:
						myStats->weapon = newItem(TOOL_PICKAXE, EXCELLENT, -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
					case 5:
					case 6:
					case 7:
					...
					case 9:
						myStats->GOLD += 100;
						myStats->weapon = newItem(MAGICSTAFF_LIGHTNING, EXCELLENT, -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
				}
			}
			...
			if ( myStats->cloak == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_CLOAK] == 1 )
			{
				switch ( map_rng.rand() % 10 )
				{
					...
					case 8:
					case 9:
						myStats->cloak = newItem(CLOAK, SERVICABLE, -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
						break;
				}
			}
		}
...
void gnomeDie(Entity* my)
{
	...
	playSoundEntity(my, 225 + map_rng.rand() % 4, 128);
