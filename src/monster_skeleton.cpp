	...
	if ( multiplayer != CLIENT && !MONSTER_INIT )
	{
		if ( myStats != nullptr )
		{
			...
			else
			{
				...
				// boss variants
				if ( local_rng.getU32() % 50 > 0 || my->flags[USERFLAG2] || strcmp(myStats->name, "") || myStats->MISC_FLAGS[STAT_FLAG_DISABLE_MINIBOSS] )
				{
					...
					if ( strncmp(map.name, "Underworld", 10) )
					{
						...
						if ( myStats->weapon == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_WEAPON] == 1 )
						{
							switch ( local_rng.getU32() % 10 )
							{
								case 0:
								case 1:
									myStats->weapon = newItem(BRONZE_AXE, WORN, -1 + local_rng.getU32() % 2, 1, local_rng.getU32(), false, nullptr);
									break;
								case 2:
								case 3:
									myStats->weapon = newItem(BRONZE_SWORD, WORN, -1 + local_rng.getU32() % 2, 1, local_rng.getU32(), false, nullptr);
									break;
								case 4:
								case 5:
									myStats->weapon = newItem(IRON_SPEAR, WORN, -1 + local_rng.getU32() % 2, 1, local_rng.getU32(), false, nullptr);
									break;
								case 6:
								case 7:
									myStats->weapon = newItem(IRON_AXE, WORN, -1 + local_rng.getU32() % 2, 1, local_rng.getU32(), false, nullptr);
									break;
								case 8:
								case 9:
									myStats->weapon = newItem(IRON_SWORD, WORN, -1 + local_rng.getU32() % 2, 1, local_rng.getU32(), false, nullptr);
									break;
							}
						}
					}
				...
				else
				{
					...
					myStats->weapon = newItem(ARTIFACT_AXE, static_cast<Status>(status), 1, 1, local_rng.getU32(), true, nullptr);
				...
				if ( myStats->helmet == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_HELM] == 1 )
				{
					switch ( local_rng.getU32() % 10 )
					{
						...
						case 4:
							break;
						case 5:
							myStats->helmet = newItem(LEATHER_HELM, DECREPIT, -1 + local_rng.getU32() % 2, 1, local_rng.getU32(), false, nullptr);
							break;
						case 6:
						case 7:
						case 8:
						case 9:
							myStats->helmet = newItem(IRON_HELM, DECREPIT, -1 + local_rng.getU32() % 2, 1, local_rng.getU32(), false, nullptr);
							break;
					}
				}
	...
	if ( multiplayer != CLIENT && my->monsterAllySummonRank != 0 )
	{
		...
		if ( leader )
		{
			...
			if ( leaderStats )
			{
				...
				if ( manaToRefund > 0 )
				{
					manaToRefund -= local_rng.getU32() % (std::max(1, manaToRefund / 3));
					if ( leaderStats->HP <= 0 )
					{
						manaToRefund = 0;
					}
