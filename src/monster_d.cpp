		...
		switch ( bodypart )
		{
			...
			case LIMB_HUMANOID_TORSO:
				...
				if ( multiplayer != CLIENT )
				{
					...
					{
						entity->sprite = itemModel(myStats->breastplate, my->sprite == 1514 || my->sprite == 1515);
					}
			...
			case LIMB_HUMANOID_MASK:
				...
				if ( entity->sprite != 165 && entity->sprite != 1196 )
				{
					...
					else if ( EquipmentModelOffsets.modelOffsetExists(MONSTER_D, entity->sprite) )
					{
						my->setHelmetLimbOffset(entity);
						my->setHelmetLimbOffsetWithMask(helmet, entity);
					}
				...
				{
					entity->focalx = limbs[MONSTER_D][10][0] + .25; // .25
					entity->focaly = limbs[MONSTER_D][10][1] - 2.25; // -2.25
					entity->focalz = limbs[MONSTER_D][10][2]; // .25
				}
				break;
			// head left
			case 12:
			{
				...
				if ( entity->skill[0] == 0 )
				{
					if ( moving )
					{
						entity->fskill[0] += std::min(dist * MONSTER_DWALKSPEED, 2.f * MONSTER_DWALKSPEED); // move proportional to move speed
					}
					else if ( my->monsterAttack != 0 )
					{
						entity->fskill[0] += MONSTER_DWALKSPEED; // move fixed speed when attacking if stationary
					}
					else
					{
