...
ItemType itemCurve(const Category cat)
{
	...
	else if ( cat == TOOL )
	{
		...
		for ( c = 0; c < numitems; c++ )
		{
			...
			if ( items[c].category == cat )
			{
				...
				{
					case TOOL_TINOPENER:
						if ( map_rng.getU32() % 2 )   // 50% chance
						{
							chances[c] = true;
						}
						break;
					case TOOL_LANTERN:
						if ( map_rng.getU32() % 4 )   // 75% chance
						{
							chances[c] = true;
						}
						break;
	...
	// pick the item
	Uint32 pick = map_rng.getU32() % numleft;
	for ( c = 0; c < numitems; c++ )
	{
		if ( items[c].category == cat )
		{
				...
				if ( cat == TOOL )
				{
					switch ( static_cast<ItemType>(c) )
					{
						case TOOL_TINOPENER:
							...
							break;
						case TOOL_LANTERN:
							if ( map_rng.getU32() % 4 == 0 )   // 25% chance
							{
								chances[c] = false;
							}
							break;
...
bool dropItem(Item* const item, const int player, const bool notifyMessage)
{
	...
	else
	{
		...
		entity->vel_x = (1.5 + .025 * (local_rng.getU32() % 11)) * cos(players[player]->entity->yaw);
		entity->vel_y = (1.5 + .025 * (local_rng.getU32() % 11)) * sin(players[player]->entity->yaw);
		entity->vel_z = (-10 - local_rng.getU32() % 20) * .01;
		...
		// play sound
		playSoundEntity( players[player]->entity, 47 + local_rng.getU32() % 3, 64 );
		...
		if ( multiplayer != CLIENT && !intro && !fadeout )
		{
			if ( players[player] != nullptr && players[player]->entity != nullptr)
			{
				if (players[player]->entity->ticks > 60)
				{
					...
					{
						playSoundEntity(players[player]->entity, 33 + local_rng.getU32() % 2, 64);
					}
					...
					{
						playSoundEntity(players[player]->entity, 40 + local_rng.getU32() % 4, 64);
					}
					else if ( itemCategory(item) == ARMOR 
						|| item->type == TOOL_TINKERING_KIT 
						|| itemTypeIsQuiver(item->type) )
					{
						playSoundEntity(players[player]->entity, 44 + local_rng.getU32() % 3, 64);
					}
...
void useItem(Item* item, const int player, Entity* usedBy, bool unequipForDropping)
{
	...
	if ( players[player]->isLocalPlayer() )
	{
			...
			&& players[player]->entity == usedBy )
		{
			...
			if ( tryEmptyBottle && local_rng.getU32() % 100 < std::min(80, (60 + skillLVL * 10)) ) // 60 - 80% chance
			{
...
Item* itemPickup(const int player, Item* const item, Item* addToSpecificInventoryItem, bool forceNewStack)
{
	...
	else
	{
		...
		if ( !appearancesOfSimilarItems.empty() )
		{
			...
			{
				robot = true;
				item->appearance += (local_rng.getU32() % 100000) * 10;
			}
			else
			{
...
Sint32 Item::potionGetEffectDurationRandom() const
{
	...
	return potionGetEffectDurationMinimum() + (local_rng.getU32() % (range));
}
...
Sint32 Item::potionGetCursedEffectDurationRandom() const
{
	...
	return potionGetCursedEffectDurationMinimum() + (local_rng.getU32() % (range));
}
			...
			&& static_cast<int>(entity->y / 16) == y )
		{
			...
			if ( entity->skill[4] == 0 )
			{
					...
					&& (stats[player]->weapon->type == TOOL_LOCKPICK || stats[player]->weapon->type == TOOL_SKELETONKEY) )
				{
					...
					bool failed = false;
					if ( skill < 2 || local_rng.getU32() % skill == 0 ) // 20 skill requirement.
					{
						// failed.
					...
					// degrade lockpick.
					if ( !(stats[player]->weapon->type == TOOL_SKELETONKEY) && (local_rng.getU32() % 10 == 0 || (failed && local_rng.getU32() % 4 == 0)) )
					{
						if ( players[player]->isLocalPlayer() )
						{
							if ( count > 1 )
					...
					if ( !failed && local_rng.getU32() % 5 == 0 )
					{
	...
	if ( stats != nullptr )
	{
		for ( i = 0; i < 6 && itemsGenerated <= itemLimit; ++i )
		{
			...
			if ( itemId >= 0 )
			{
				...
				if ( itemBless == 10 )
				{
					itemBless = -1 + local_rng.getU32() % 3;
				}
...
bool swapMonsterWeaponWithInventoryItem(Entity* const my, Stat* const myStats, node_t* const inventoryNode, const bool moveStack,  const bool overrideCursed)
{
	...
	if ( item->count == 1 || moveStack )
	{
		...
		if ( myStats->weapon != nullptr )
		{
			...
			{
				playSoundEntity(my, 40 + local_rng.getU32() % 4, 64);
			}
			free(tmpItem);
		}
		else
...
ItemType itemTypeWithinGoldValue(const int cat, const int minValue, const int maxValue)
{
	...
	// pick the item
	int pick = local_rng.getU32() % numoftype;// map_rng.getU32() % numoftype;
	for ( c = 0; c < numitems; c++ )
	{
		if ( chances[c] == true )
		{
