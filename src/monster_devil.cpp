...
void initDevil(Entity* my, Stat* myStats)
{
	...
	if ( multiplayer != CLIENT && !MONSTER_INIT )
	{
		...
		for ( c = 0; c < std::max<DOUBLE>(NUMPROFICIENCIES, NUMEFFECTS); c++ )
		{
	...
	for (bodypart = 0, node = my->children.first; node != NULL; node = node->next, bodypart++)
	{
		...
		if ( bodypart == 3 )
		{
			if ( MONSTER_ATTACK == 1 || MONSTER_ATTACK == 3 )
			{
				...
				{
					entity->pitch = std::max<DOUBLE>(entity->pitch - .1, -PI / 2);
				}
				else if ( MONSTER_ATTACKTIME > 40 )
				{
					if ( entity->pitch < 0 )
					{
						entity->pitch = std::min<DOUBLE>(entity->pitch + .4, 0.0);
						if ( entity->pitch >= 0 )
						{
							playSound(181, 64);
						}
			...
			{
				entity->pitch = std::max<DOUBLE>(entity->pitch - .1, -4 * PI / 5);
			}
			...
			case 2:
			{
				...
				else
				{
					...
					else
					{
						if ( MONSTER_ATTACK == 1 )
						{
							entity->yaw = std::min<DOUBLE>(entity->yaw + .1, my->yaw + PI / 6);
						}
						else if ( MONSTER_ATTACK == 2 )
						{
							entity->yaw = std::max<DOUBLE>(entity->yaw - .1, my->yaw - PI / 6);
						}
					}
				}
				if ( MONSTER_ATTACK == 4 )
				{
					entity->pitch = std::max<DOUBLE>(entity->pitch - .1, -PI / 6);
				}
				else
				{
					entity->pitch = std::min<DOUBLE>(entity->pitch + .1, 0.0);
				}
				break;
			}
			// right bicep
