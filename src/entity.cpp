				...
				switch ( c )
				{
						...
						if ( vampiricAura_hijacked )
						{
							...
							if ( caster )
							{
								...
								if ( deducted )
								{
									...
									if ( caster->behavior == &actMonster && local_rng.getU32() % 20 == 0 )
									{
										sustained = false;
					...
					case EFF_WITHDRAWAL:
						if ( player >= 0 && player < MAXPLAYERS )
						{
							...
							{
								playSoundPlayer(player, 32, 128);
								messagePlayer(player, MESSAGE_STATUS, language[3247 + local_rng.getU32() % 3]);
...
void Entity::handleEffects(Stat* myStats)
{
	...
	if ( myStats->EXP >= 100 )
	{
		...
		{
			// monsters use this.
			increasestat[0] = local_rng.getU32() % 6;
			int r = local_rng.getU32() % 6;
			while ( r == increasestat[0] ) {
				r = local_rng.getU32() % 6;
			}
			increasestat[1] = r;
			r = local_rng.getU32() % 6;
	...
	{
		if ( myStats->HUNGER > 1500 && local_rng.getU32() % 1000 == 0 )
		{
			// oversatiation
			...
			else
			{
				...
				this->char_gonnavomit = 140 + local_rng.getU32() % 60;
			}
		}
		else if ( ticks % 60 == 0 && local_rng.getU32() % 200 == 0 && myStats->EFFECTS[EFF_DRUNK] && myStats->type != GOATMAN )
		{
			// drunkenness
			messagePlayer(player, MESSAGE_STATUS, language[634]);
			this->char_gonnavomit = 140 + local_rng.getU32() % 60;
	...
	if ( this->char_gonnavomit > 0 )
	{
		...
		if ( this->char_gonnavomit == 0 )
		{
			...
			myStats->EFFECTS_TIMERS[EFF_VOMITING] = 50 + local_rng.getU32() % 20;
			serverUpdateEffects(player);
			if ( player >= 0 && players[player]->isLocalPlayer() )
			{
				camera_shakey += 9;
	...
	if ( myStats->EFFECTS[EFF_VOMITING] && ticks % 2 == 0 )
	{
		...
		if ( entity )
		{
			...
			entity->yaw = this->yaw - 0.1 + (local_rng.getU32() % 20) * 0.01;
			entity->pitch = (local_rng.getU32() % 360) * PI / 180.0;
			entity->roll = (local_rng.getU32() % 360) * PI / 180.0;
			double vel = (local_rng.getU32() % 15) / 10.f;
	...
	if ( myStats->ring != NULL )
	{
		...
		{
			if ( local_rng.getU32() % 1000 == 0 )   // .1% chance every frame
			{
				teleportRandom();
			}
		}
	...
	if ( myStats->EFFECTS[EFF_POISONED] )
	{
		...
		if ( this->char_poison > 150 )   // three seconds
		{
			...
			if ( poisonhurt > 3 )
			{
				poisonhurt -= local_rng.getU32() % (std::max(1, poisonhurt / 4));
			}
			...
			if ( local_rng.getU32() % 5 == 0 && getCON() >= -3 )
			{
	...
	if ( myStats->EFFECTS[EFF_BLEEDING] )
	{
		if ( ticks % 120 == 0 )
		{
			if ( myStats->HP > 5 + (std::max(0, getCON())) ) // CON increases when bleeding stops.
			{
				...
				if ( bleedhurt > 1 )
				{
					bleedhurt -= local_rng.getU32() % (std::max(1, bleedhurt / 2));
				}
				if ( getCON() > 0 )
				{
					bleedhurt -= (getCON() / 5);
	...
	if ( myStats->EFFECTS[EFF_WEBBED] )
	{
		...
		if ( ticks % 40 == 0 )
		{
			...
			if ( entity != NULL )
			{
				...
				entity->sizey = 2;
				entity->yaw = (local_rng.getU32() % 360) * PI / 180.0;
				real_t scale = 0.75 + 0.25 * (local_rng.getU32() % 100) / 100.f;
				entity->scalex = scale;
				entity->scaley = scale;
				entity->flags[UPDATENEEDED] = true;
				entity->flags[PASSABLE] = true;
	...
	if ( myStats->EFFECTS[EFF_MAGICREFLECT] )
	{
		spawnAmbientParticles(80, 579, 10 + local_rng.getU32() % 40, 1.0, false);
	}
	...
	if ( myStats->EFFECTS[EFF_FEAR] )
	{
		...
		{
			spawnAmbientParticles(1, 864, 20 + local_rng.getU32() % 10, 0.5, true);
		}
	}

	if ( myStats->EFFECTS[EFF_TROLLS_BLOOD] )
	{
		spawnAmbientParticles(80, 169, 20 + local_rng.getU32() % 10, 0.5, true);
	}
	...
	if ( myStats->EFFECTS[EFF_PACIFY] )
	{
		if ( ticks % 25 == 0 || ticks % 40 == 0 )
		{
			spawnAmbientParticles(1, 685, 20 + local_rng.getU32() % 10, 0.5, true);
		}
	}
	...
	if ( myStats->EFFECTS[EFF_SHADOW_TAGGED] )
	{
		if ( ticks % 25 == 0 || ticks % 40 == 0 )
		{
			spawnAmbientParticles(1, 871, 20 + local_rng.getU32() % 10, 0.5, true);
		}
	}
	...
	if ( myStats->EFFECTS[EFF_POLYMORPH] )
	{
		if ( ticks % 25 == 0 || ticks % 40 == 0 )
		{
			spawnAmbientParticles(1, 593, 20 + local_rng.getU32() % 10, 0.5, true);
		}
	}

	if ( myStats->EFFECTS[EFF_INVISIBLE] && myStats->type == SHADOW )
	{
		spawnAmbientParticles(20, 175, 20 + local_rng.getU32() % 30, 0.5, true);
	}
	...
	if ( this->flags[BURNING] )
	{
		...
		else
		{
			...
			if ( (this->char_fire % TICKS_TO_PROCESS_FIRE) == 0 )
			{
				...
				if ( buddhamode )
				{
					Sint32 fireDamage = (-2 - local_rng.getU32() % 3); // Deal between -2 to -5 damage
				...
				else
				{
					...
					this->modHP(-2 - local_rng.getU32() % 3); // Deal between -2 to -5 damage
				...
				if ( myStats->cloak != nullptr )
				{
					...
					if ( local_rng.getU32() % 10 == 0 && myStats->cloak->type != ARTIFACT_CLOAK && myStats->cloak->type != CLOAK_BACKPACK )
					{
						if ( player >= 0 && players[player]->isLocalPlayer() )
						{
				...
				if ( (local_rng.getU32() % this->chanceToPutOutFire) == 0 )
				{
					this->flags[BURNING] = false;
					messagePlayer(player, MESSAGE_STATUS, language[647]); // "The flames go out."
					serverUpdateEntityFlag(this, BURNING);
	...
	if ( multiplayer != CLIENT )
	{
			...
			if ( aoeTargets )
			{
				for ( node = aoeTargets->first; node != NULL; node = node->next )
				{
					...
					if ( tmpEntity != nullptr )
					{
						...
						if ( tmpStats )
						{
							int explodeDmg = (10 + local_rng.getU32() % 10 + myStats->LVL) * tmpEntity->getDamageTableMultiplier(*tmpStats, DAMAGE_TABLE_MAGIC); // check base magic damage resist.
			...
			&& (!shapeshifted || (shapeshifted && myStats->type == CREATURE_IMP && itemCategory(myStats->weapon) == MAGICSTAFF)) )
		{
			...
			if ( itemCategory(myStats->weapon) == SPELLBOOK || itemCategory(myStats->weapon) == MAGICSTAFF )
			{
				if ( itemCategory(myStats->weapon) == MAGICSTAFF )
				{
					...
					if ( (local_rng.getU32() % 3 == 0 && degradeWeapon && !(svFlags & SV_FLAG_HARDCORE)) || forceDegrade
						|| ((svFlags & SV_FLAG_HARDCORE) && local_rng.getU32() % 6 == 0 && degradeWeapon) )
					{
						if ( player >= 0 && players[player]->isLocalPlayer() )
						{
			...
			else if ( isRangedWeapon(*myStats->weapon) )
			{
				...
				if ( bowDegradeChance < 100 && local_rng.getU32() % bowDegradeChance == 0 && myStats->weapon->type != ARTIFACT_BOW )
				{
					if ( myStats->weapon != NULL )
					{
						if ( player >= 0 && players[player]->isLocalPlayer() )
				...
				if ( myStats->weapon->type == SLING )
				{
					...
					playSoundEntity(this, 239 + local_rng.getU32() % 3, 96);
				}
				else if ( myStats->weapon->type == CROSSBOW || myStats->weapon->type == HEAVY_CROSSBOW )
				{
					...
					{
						playSoundEntity(this, 411 + local_rng.getU32() % 3, 128);
				...
				{
					entity->setRangedProjectileAttack(*this, *myStats, QUIVER_SILVER + local_rng.getU32() % 7);
				}
				else
				{
				...
				|| itemIsThrowableTinkerTool(myStats->weapon) )
			{
				...
				if ( myStats->weapon->type == BOOMERANG )
				{
					...
					//playSoundEntity(this, 427 + local_rng.getU32() % 4, 128);
				...
				else if ( itemIsThrowableTinkerTool(myStats->weapon) )
				{
					...
					entity->vel_z = -.3;
					entity->roll -= (PI / 2 - 0.1 + (local_rng.getU32() % 10) * 0.02);
		...
		if ( target == nullptr )
		{
			if ( whip )
			{
				...
				playSoundEntity(this, 23 + local_rng.getU32() % 5, 128); // whoosh noise
			}
			else
			{
				playSoundEntity(this, 23 + local_rng.getU32() % 5, 128); // whoosh noise
		...
		if ( hit.entity != nullptr )
		{
			...
			if ( hit.entity->behavior == &actBoulder )
			{
				if ( myStats->weapon != nullptr || pose == PLAYER_POSE_GOLEM_SMASH )
				{
					if ( (myStats->weapon && myStats->weapon->type == TOOL_PICKAXE) || pose == PLAYER_POSE_GOLEM_SMASH )
					{
						...
						if ( myStats->weapon && local_rng.getU32() % 2 && pose != PLAYER_POSE_GOLEM_SMASH )
						{
							myStats->weapon->status = static_cast<Status>(myStats->weapon->status - 1);
							if ( myStats->weapon->status < BROKEN )
							{
			...
			else if ( hit.entity->behavior == &actDoor || hit.entity->behavior == &::actFurniture || hit.entity->behavior == &::actChest )
			{
				...
				if ( whip )
				{
					playSoundEntity(hit.entity, 407 + local_rng.getU32() % 3, 64);
				}
				else
				{
			...
			else if ( hit.entity->behavior == &actSink )
			{
				...
				playSoundEntity(hit.entity, 140 + local_rng.getU32(), 64);
			...
			if ( hitstats != nullptr )
			{
				...
				c = local_rng.getU32()%20 + hitskill + (weaponskill==PRO_POLEARM);
				...
				if( hitsuccess )*/
				{
					...
					if ( myStats->type == LICH_FIRE && !hitstats->defending )
					{
						if ( damage <= 8 )
						{
							damage += (8 - damage) + local_rng.getU32() % 9; // 8 - 16 minimum damage.
						}
					}
					...
					if ( (weaponskill >= PRO_SWORD && weaponskill < PRO_SHIELD && !gungnir) || (weaponskill == PRO_UNARMED && behavior != &actMonster) || weaponskill == PRO_RANGED )
					{
						...
						if ( chance > 0 )
						{
							damage = (damage - chance) + (local_rng.getU32() % chance) + 1;
						}
					}
					...
					if ( weaponToBreak != nullptr && !shapeshifted )
					{
						...
						if ( !artifactWeapon )
						{
							...
							if	( (local_rng.getU32() % degradeOnZeroDMG == 0 && damage == 0)
									|| (local_rng.getU32() % degradeOnNormalDMG == 0 && damage > 0)
								)
							{
								degradeWeapon = true;
							}
					...
					if ( damage > 0 || (damage == 0 && !(hitstats->shield && hitstats->defending)) )
					{
						...
						{
							if ( armorDegradeChance == 100 || (local_rng.getU32() % armorDegradeChance > 0) )
							{
								armor = NULL;
								armornum = 0;
							}
						...
						&& hitstats->shield->type != TOOL_TINKERING_KIT )
					{
						...
						else
						{
							...
							{
								if ( (local_rng.getU32() % 15 == 0 && damage > 0) || (damage == 0 && local_rng.getU32() % 8 == 0) )
								{
							...
							if ( shieldDegradeChance < 100 && armor == NULL &&
								(	(hitstats->defending && local_rng.getU32() % shieldDegradeChance == 0)
									|| (hitstats->defending && pose == MONSTER_POSE_GOLEM_SMASH && target == nullptr && local_rng.getU32() % 3 == 0)
								)
								)
							{
								armor = hitstats->shield;
					...
					if ( myStats->weapon && !shapeshifted )
					{
						if ( myStats->weapon->type == ARTIFACT_SWORD )
						{
							real_t amount = 0.0;
							real_t percent = getArtifactWeaponEffectChance(myStats->weapon->type, *myStats, &amount);
							if ( dyrnwynSmite || (local_rng.getU32() % 100 < static_cast<int>(percent)) )
							{
					...
					else if ( damage > 0 && behavior == &actPlayer && weaponskill >= PRO_SWORD && weaponskill <= PRO_POLEARM && (charge >= MAXCHARGE - 3) )
					{
						...
						else if ( weaponskill == PRO_MACE && hitstats->HP > 0 )
						{
							...
							{
								if ( local_rng.getU32() % chance == 0 && !hitstats->EFFECTS[EFF_PARALYZED] )
								{
					...
					else if ( (damage > 0 || hitstats->EFFECTS[EFF_PACIFY] || hitstats->EFFECTS[EFF_FEAR]) && local_rng.getU32() % 4 == 0 )
					{
						int armornum = 0;
						Item* armor = nullptr;
						int armorstolen = local_rng.getU32() % 9;
						switch ( myStats->type )
						{
							case SCORPION:
					...
					if ( hitstats->HP > 0 )
					{
						if ( !whip && hitstats->EFFECTS[EFF_DISORIENTED] )
						{
							...
							|| !hit.entity->isMobile()
							|| (hitstats->EFFECTS[EFF_DRUNK] && local_rng.getU32() % 3 == 0)
							|| (hitstats->EFFECTS[EFF_CONFUSED] && local_rng.getU32() % 3 == 0))
							)
						{
							if ( hit.entity->behavior == &actMonster && !hit.entity->isBossMonster() )
							{
									...
									&& itemCategory(hitstats->weapon) != SPELLBOOK )
								{
									...
									if ( dropped )
									{
										...
										dropped->vel_x = (1.5 + .025 * (local_rng.getU32() % 11)) * cos(tangent);
										dropped->vel_y = (1.5 + .025 * (local_rng.getU32() % 11)) * sin(tangent);
					...
					if ( gibtype[(int)hitstats->type] > 0 )
					{
						...
						{
							if ( bleedStatusInflicted || (local_rng.getU32() % 20 == 0 && (weaponskill > PRO_SWORD && weaponskill <= PRO_POLEARM) )
								|| (local_rng.getU32() % 10 == 0 && weaponskill == PRO_SWORD)
								|| (whip && ( (flanking && local_rng.getU32() % 5 == 0) || (backstab && local_rng.getU32() % 2 == 0) || disarmed) )
								|| (local_rng.getU32() % 4 == 0 && pose == MONSTER_POSE_GOLEM_SMASH)
								|| (local_rng.getU32() % 4 == 0 && pose == PLAYER_POSE_GOLEM_SMASH)
								|| (local_rng.getU32() % 10 == 0 && myStats->type == VAMPIRE && myStats->weapon == nullptr)
								|| (local_rng.getU32() % 8 == 0 && myStats->EFFECTS[EFF_VAMPIRICAURA] && (myStats->weapon == nullptr || myStats->type == LICH_FIRE))
							)
							{
								...
								{
									if ( local_rng.getU32() % 2 == 0 ) // 50% for heavy bleed effect.
									{
										heavyBleedEffect = true;
									}
								}
								...
								if ( (!wasBleeding && !heavyBleedEffect) || bleedStatusInflicted )
								{
									...
									{
										hitstats->EFFECTS_TIMERS[EFF_BLEEDING] = std::max(480 + (int)local_rng.getU32() % 360 - hit.entity->getCON() * 100, 120); // 2.4-16.8 seconds
									}
								...
								else if ( heavyBleedEffect )
								{
									if ( !wasBleeding )
									{
										hitstats->EFFECTS_TIMERS[EFF_BLEEDING] = std::max(500 + (int)local_rng.getU32() % 500 - hit.entity->getCON() * 10, 250); // 5-20 seconds
										hitstats->EFFECTS[EFF_BLEEDING] = true;
									...
									{
										hitstats->EFFECTS_TIMERS[EFF_BLEEDING] += std::max((int)local_rng.getU32() % 350 - hit.entity->getCON() * 5, 100); // 2-7 seconds in addition
										hitstats->EFFECTS[EFF_BLEEDING] = true;
								...
								if ( hitstats->EFFECTS[EFF_BLEEDING] && (!wasBleeding || heavyBleedEffect) )
								{
									...
									{
										this->modMP(1 + local_rng.getU32() % 2);
										Uint32 color = makeColorRGB(0, 255, 0);
					...
					if ( tryLifesteal || forceLifesteal )
					{
						...
						else if ( (local_rng.getU32() % 4 == 0) && (myStats->type == VAMPIRE && behavior == &actMonster && myStats->EFFECTS[EFF_VAMPIRICAURA]) )
						{
					...
					if ( tryBloodVial )
					{
						...
						else if ( hitstats->EFFECTS[EFF_BLEEDING] || myStats->EFFECTS[EFF_VAMPIRICAURA] )
						{
							...
							{
								spawnBloodVial = (local_rng.getU32() % 2 == 0);
							}
							else if ( hitstats->EFFECTS_TIMERS[EFF_BLEEDING] >= 150 )
							{
								spawnBloodVial = (local_rng.getU32() % 4 == 0);
							}
							else
							{
								spawnBloodVial = (local_rng.getU32() % 8 == 0);
							}
						...
						{
							spawnBloodVial = (local_rng.getU32() % 10 == 0);
						}
				...
				if ( pose == PLAYER_POSE_GOLEM_SMASH )
				{
					if ( hit.mapx >= 1 && hit.mapx < map.width - 1 && hit.mapy >= 1 && hit.mapy < map.height - 1 )
					{
						...
						for ( int c = 0; c < 5; c++ )
						{
							...
							entity->sizex = 1;
							entity->sizey = 1;
							entity->x = hit.x + (-4 + local_rng.getU32() % 9);
							entity->y = hit.y + (-4 + local_rng.getU32() % 9);
							entity->z = 7.5;
					...
					if ( myStats->weapon->type == TOOL_PICKAXE )
					{
						if ( hit.mapx >= 1 && hit.mapx < map.width - 1 && hit.mapy >= 1 && hit.mapy < map.height - 1 )
						{
							...
							else
							{
								playSoundPos(hit.x, hit.y, 67, 128); // bust wall
								// spawn several rock items
								i = 8 + local_rng.getU32() % 4;
								for ( c = 0; c < i; c++ )
								{
									Entity* entity = newEntity(-1, 1, map.entities, nullptr); //Rock/item entity.
									entity->flags[INVISIBLE] = true;
									entity->flags[UPDATENEEDED] = true;
									entity->x = hit.mapx * 16 + 4 + local_rng.getU32() % 8;
									entity->y = hit.mapy * 16 + 4 + local_rng.getU32() % 8;
									entity->z = -6 + local_rng.getU32() % 12;
									entity->sizex = 4;
									entity->sizey = 4;
									entity->yaw = local_rng.getU32() % 360 * PI / 180;
							...
							if ( local_rng.getU32() % chance && degradePickaxe )
							{
								myStats->weapon->status = static_cast<Status>(myStats->weapon->status - 1);
								if ( myStats->weapon->status == BROKEN )
								{
...
bool Entity::teleportRandom()
{
	...
	pickedlocation = local_rng.getU32() % numlocations;
	numlocations = 0;
	for ( int iy = 1; iy < map.height; iy++ )
	{
		for ( int ix = 1; ix < map.width; ix++ )
...
bool Entity::teleportAroundEntity(Entity* target, int dist, int effectType)
{
	...
	{
		tmpPair = goodspots[local_rng.getU32() % goodspots.size()];
	}
	else
	{
		tmpPair = spotsBehindMonster[local_rng.getU32() % spotsBehindMonster.size()];
	}
	tx = tmpPair.first;
	ty = tmpPair.second;
	if ( behavior == &actPlayer )
...
void Entity::awardXP(Entity* src, bool share, bool root)
{
	...
	// calculate XP gain
	int baseXp = 10;
	int xpGain = baseXp + local_rng.getU32() % std::max(1, baseXp) + std::max(0, srcStats->LVL - destStats->LVL) * baseXp;
	...
	if ( player >= 0 )
	{
		...
		if ( root )
		{
			...
			if ( guerillaRadio )
			{
				...
				if ( local_rng.getU32() % 5 == 0 || (uidToEntity(src->monsterTarget) != this && local_rng.getU32() % 3 == 0) )
				{
...
void createMonsterEquipment(Stat* stats)
{
	...
	Status itemStatus;
	int itemBless;
	int itemAppearance = local_rng.getU32();
	int itemCount;
	int chance = 1;
	int category = 0;
	bool itemIdentified;
	if ( stats != nullptr )
	{
		for ( itemIndex = 0; itemIndex < 10; ++itemIndex )
		{
			...
			if ( itemId >= 0 && generateItem )
			{
				...
				if ( itemStatus == 0 )
				{
					itemStatus = static_cast<Status>(DECREPIT + local_rng.getU32() % 4);
				}
				...
				if ( itemBless == 10 )
				{
					itemBless = -2 + local_rng.getU32() % 5;
				}
				...
				{
					itemIdentified = local_rng.getU32() % 2;
				}
				else
				{
					itemIdentified = false;
				}
				itemAppearance = local_rng.getU32();
				chance = stats->EDITOR_ITEMS[itemIndex * ITEM_SLOT_NUMPROPERTIES + 5];

				if ( local_rng.getU32() % 100 < chance )
				{
					switch ( itemIndex ) {
						case 0:
...
void setRandomMonsterStats(Stat* stats)
{
	if ( stats != nullptr )
	{
		...
		{
			stats->MAXHP += local_rng.getU32() % (stats->RANDOM_MAXHP + 1);
			...
			{
				// roll the current hp
				stats->HP += local_rng.getU32() % (stats->RANDOM_HP + 1);
			}
		}
		else
		{
		...
		{
			stats->MAXMP += local_rng.getU32() % (stats->RANDOM_MAXMP + 1);
			...
			{
				// roll the current mp
				stats->MP += local_rng.getU32() % (stats->RANDOM_MP + 1);
			}
		}
		else
		{
		...
		stats->STR += local_rng.getU32() % (stats->RANDOM_STR + 1);
		stats->DEX += local_rng.getU32() % (stats->RANDOM_DEX + 1);
		stats->CON += local_rng.getU32() % (stats->RANDOM_CON + 1);
		stats->INT += local_rng.getU32() % (stats->RANDOM_INT + 1);
		stats->PER += local_rng.getU32() % (stats->RANDOM_PER + 1);
		stats->CHR += local_rng.getU32() % (stats->RANDOM_CHR + 1);

		stats->LVL += local_rng.getU32() % (stats->RANDOM_LVL + 1);
		stats->GOLD += local_rng.getU32() % (stats->RANDOM_GOLD + 1);
	}
		...
		else
		{
				...
				|| myStats->type == SHADOW )
			{
					...
					|| myStats->weapon->type == TOOL_WHIP )
				{
					...
					pose = MONSTER_POSE_MELEE_WINDUP1 + local_rng.getU32() % 2;
				}
				else
				{
					pose = MONSTER_POSE_MELEE_WINDUP1 + local_rng.getU32() % 3;
				}
			}
			else
			{
				pose = local_rng.getU32() % 3 + 1;
			}
		}
	}
	// fists
...
void Entity::spawnAmbientParticles(int chance, int particleSprite, int duration, double particleScale, bool shrink)
{
	if ( local_rng.getU32() % chance == 0 )
	{
		...
		spawnParticle->x = x + (-2 + local_rng.getU32() % 5);
		spawnParticle->y = y + (-2 + local_rng.getU32() % 5);
		spawnParticle->z = 7.5;
...
void Entity::handleEffectsClient()
{
	...
	if ( myStats->EFFECTS[EFF_VAMPIRICAURA] )
	{
		spawnAmbientParticles(30, 600, 20 + local_rng.getU32() % 30, 0.5, true);
	}

	if ( myStats->EFFECTS[EFF_PACIFY] )
	{
		spawnAmbientParticles(30, 685, 20 + local_rng.getU32() % 30, 0.5, true);
	}
...
void Entity::monsterAcquireAttackTarget(const Entity& target, Sint32 state, bool monsterWasHit)
{
	...
	if ( myStats->type == SHOPKEEPER && monsterTarget != target.getUID() && target.behavior == &actPlayer )
	{
		...
		if ( targetStats )
		{
			...
			{
				messagePlayer(target.skill[2], MESSAGE_WORLD, language[516 + local_rng.getU32() % 4], namesays);
			}
		...
		for ( node = itemsList->first; node != nullptr; node = node->next )
		{
			...
			if ( node->element )
			{
				...
				if ( myStats->type == SHOPKEEPER && myStats->MISC_FLAGS[STAT_FLAG_MYSTERIOUS_SHOPKEEP] > 0 )
				{
					...
					playSoundEntity(this, 35 + local_rng.getU32() % 3, 64);
				...
				else if ( shouldWield )
				{
					...
					if ( item && item->interactNPCUid == getUID() && forcePickupItem )
					{
						...
						{
							playSoundEntity(this, 33 + local_rng.getU32() % 2, 64);
						}
						else if ( itemCategory(item) == WEAPON || itemCategory(item) == THROWN )
						{
							playSoundEntity(this, 40 + local_rng.getU32() % 4, 64);
						}
						else if ( itemCategory(item) == ARMOR )
						{
							playSoundEntity(this, 44 + local_rng.getU32() % 3, 64);
						}
...
node_t* Entity::chooseAttackSpellbookFromInventory()
{
	...
	spellbook = spellbookNodeInInventory(myStats, spellbooks[local_rng.getU32()%spellbooks.size()]); //Choose a random spell and return it.
	if (!spellbook )
	{
...
void Entity::setRangedProjectileAttack(Entity& marksman, Stat& myStats, int optionalOverrideForArrowType)
{
	...
	if ( myStats.weapon )
	{
		...
		if ( myStats.weapon->type != SLING )
		{
			...
			int chance = local_rng.getU32() % 100;
	...
	if ( chance > 0 )
	{
		attack = (attack - chance) + (local_rng.getU32() % chance) + 1;
	}
...
void Entity::setHardcoreStats(Stat& stats)
{
	if ( (svFlags & SV_FLAG_HARDCORE) && stats.MISC_FLAGS[STAT_FLAG_MONSTER_DISABLE_HC_SCALING] == 0 )
	{
		...
		stats.HP += statIncrease - (local_rng.getU32() % (std::max(statIncrease / 5, 1))); // 80%-100% of increased value
		stats.MAXHP = stats.HP;
		stats.OLDHP = stats.HP;

		statIncrease = (abs(stats.STR) / 5 + 1) * 5; // each 5 STR add 5 more STR.
		stats.STR += (statIncrease - (local_rng.getU32() % (std::max(statIncrease / 4, 1)))); // 75%-100% of increased value.

		statIncrease = (abs(stats.PER) / 5 + 1) * 5; // each 5 PER add 5 more PER.
		stats.PER += (statIncrease - (local_rng.getU32() % (std::max(statIncrease / 4, 1)))); // 75%-100% of increased value.

		statIncrease = std::min((abs(stats.DEX) / 4 + 1) * 1, 8); // each 4 DEX add 1 more DEX, capped at 8.
		stats.DEX += (statIncrease - (local_rng.getU32() % (std::max(statIncrease / 2, 1)))); // 50%-100% of increased value.

		statIncrease = (abs(stats.CON) / 5 + 1) * 1; // each 5 CON add 1 more CON.
		stats.CON += (statIncrease - (local_rng.getU32() % (std::max(statIncrease / 2, 1)))); // 50%-100% of increased value.

		statIncrease = (abs(stats.INT) / 5 + 1) * 5; // each 5 INT add 5 more INT.
		stats.INT += (statIncrease - (local_rng.getU32() % (std::max(statIncrease / 2, 1)))); // 50%-100% of increased value.

		int lvlIncrease = local_rng.getU32() % 4;
...
void Entity::handleKnockbackDamage(Stat& myStats, Entity* knockedInto)
{
	...
	{
		int damageOnHit = 5 + local_rng.getU32() % 6;
...
int monsterTinkeringConvertAppearanceToHP(Stat* myStats, int appearance)
{
	if ( myStats )
	{
		...
		randomHP = randomHP + local_rng.getU32() % randomHP;
