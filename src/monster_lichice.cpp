...
void initLichIce(Entity* my, Stat* myStats)
{
	...
	if ( multiplayer != CLIENT && !MONSTER_INIT )
	{
		if ( myStats != nullptr )
		{
			...
			{
				myStats->weapon = newItem(MAGICSTAFF_COLD, EXCELLENT, -5, 1, local_rng.getU32(), false, NULL);
			}
		}
	}
					...
					if ( my->monsterAnimationLimbOvershoot != ANIMATE_OVERSHOOT_TO_SETPOINT )
					{
						...
						if ( my->monsterAttackTime >= 50 / (monsterGlobalAnimationMultiplier / 10.0) )
						{
							if ( multiplayer != CLIENT )
							{
								...
								for ( int i = 0; i < 8; ++i )
								{
									my->castFallingMagicMissile(SPELL_COLD, 16 + local_rng.getU32() % 8, dir + i * PI / 4, 0);
								}
							}
						}
					}
							...
							if ( multiplayer != CLIENT )
							{
								...
								if ( my->monsterState == MONSTER_STATE_LICHICE_DIE )
								{
									...
									for ( int i = 0; i < 8; ++i )
									{
										...
										spell->yaw += i * PI / 4 + ((PI * (-4 + local_rng.getU32() % 9)) / 64);
								...
								else
								{
									...
									if ( local_rng.getU32() % 5 == 0 || (my->monsterLichAllyStatus == LICH_ALLY_DEAD && local_rng.getU32() % 2 == 0) )
									{
						...
						if ( my->monsterAttackTime >= 50 / (monsterGlobalAnimationMultiplier / 10.0) )
						{
							if ( multiplayer != CLIENT )
							{
								...
								for ( int i = 0; i < 3; ++i )
								{
									...
									{
										// do some minor variations in spell angle
										spell->yaw += ((PI * (-4 + local_rng.getU32() % 9)) / 40);
									}
									Entity* target = uidToEntity(my->monsterTarget);
									if ( target )
									{
	...
	switch ( monsterLichIceCastSeq )
	{
		case LICH_ATK_VERTICAL_SINGLE:
			if ( monsterSpecialState == 0 && monsterState != MONSTER_STATE_LICH_CASTSPELLS
				&& monsterSpecialTimer == 0 && local_rng.getU32() % 4 > 0 )
			{
