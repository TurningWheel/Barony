...
void initLichIce(Entity* my, Stat* myStats)
{
	...
	// right arm
	Entity* entity = newEntity(653, 0, map.entities);
	entity->sizex = 4;
	entity->sizey = 4;
	...
	// left arm
	entity = newEntity(652, 0, map.entities);
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
	entity->flags[PASSABLE] = true;
	...
	// head
	entity = newEntity(651, 0, map.entities);
	entity->yaw = my->yaw;
	entity->sizex = 4;
	entity->sizey = 4;
	entity->skill[2] = my->getUID();
					...
					if ( my->monsterAnimationLimbOvershoot != ANIMATE_OVERSHOOT_TO_SETPOINT )
					{
						...
						if ( my->monsterAttackTime >= 50 / (monsterGlobalAnimationMultiplier / 10.0) )
						{
							if ( multiplayer != CLIENT )
							{
								...
								for ( int i = 0; i < 8; ++i )
								{
									my->castFallingMagicMissile(SPELL_COLD, 16 + rand() % 8, dir + i * PI / 4);
								}
							}
						}
					}
				...
				if ( spellarm->skill[1] == 0 )
				{
					...
					if ( limbAnimateToLimit(spellarm, ANIMATE_PITCH, 0.4, PI / 2, false, 0.0) )
					{
						...
						if ( multiplayer != CLIENT )
						{
							if ( rand() % 2 )
							{
								castSpell(my->getUID(), getSpellFromID(SPELL_COLD), true, false);
							}
							else
							{
