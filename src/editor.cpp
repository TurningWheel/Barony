...
void mainLogic(void) {
	...
	switch( drawlayer ) {
		case 0: strcpy(layerstatus,"FLOOR"); break;
		case 1: strcpy(layerstatus,"WALLS"); break;
		case 2: strcpy(layerstatus,"CEILING"); break;
...
void makeUndo() {
	...
	undomap->entities->first = NULL; undomap->entities->last = NULL;
	...
	while(mainloop) {
		...
		if( spritepalette ) {
			...
			for( c=0; c<numsprites; c++ ) {
				if( sprites[c] != NULL ) {
					pos.x=x; pos.y=y;
					pos.w=sprites[c]->w*2; pos.h=sprites[c]->h*2;
