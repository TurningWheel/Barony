...
void initAutomaton(Entity* my, Stat* myStats)
{
	...
	if ( !MONSTER_INIT )
	{
		if ( myStats != NULL )
		{
			...
			// boss variants
			//if ( map_rng.rand() % 50 || my->flags[USERFLAG2] )
			//{
			...
			//			case 0:
			//			case 1:
			//				//myStats->weapon = newItem(BRONZE_AXE, WORN, -1 + map_rng.rand() % 2, 1, map_rng.rand(), false, NULL);
			//				break;
			//			case 2:
			//			case 3:
			//				//myStats->weapon = newItem(BRONZE_SWORD, WORN, -1 + map_rng.rand() % 2, 1, map_rng.rand(), false, NULL);
			//				break;
			//			case 4:
			//			case 5:
			//				//myStats->weapon = newItem(IRON_SPEAR, WORN, -1 + map_rng.rand() % 2, 1, map_rng.rand(), false, NULL);
			//				break;
			//			case 6:
			//			case 7:
			//				//myStats->weapon = newItem(IRON_AXE, WORN, -1 + map_rng.rand() % 2, 1, map_rng.rand(), false, NULL);
			//				break;
			//			case 8:
			//			case 9:
			//				//myStats->weapon = newItem(IRON_SWORD, WORN, -1 + map_rng.rand() % 2, 1, map_rng.rand(), false, NULL);
			//				break;
			//		}
			//	}
			//}
			...
			//	myStats->weapon = newItem(ARTIFACT_AXE, EXCELLENT, 1, 1, map_rng.rand(), true, NULL);
			...
			if ( myStats->weapon == NULL && myStats->EDITOR_ITEMS[ITEM_SLOT_WEAPON] == 1 )
			{
				if ( greaterMonster )
				{
					...
					{
						case 0:
							myStats->weapon = newItem(MAGICSTAFF_LIGHTNING, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
							break;
						case 1:
							myStats->weapon = newItem(CRYSTAL_SPEAR, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
							break;
						case 2:
							myStats->weapon = newItem(SHORTBOW, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
							break;
						case 3:
							myStats->weapon = newItem(CROSSBOW, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
							break;
						default:
							break;
					}
			...
			if ( myStats->helmet == NULL && myStats->EDITOR_ITEMS[ITEM_SLOT_HELM] == 1 )
			{
				switch ( map_rng.rand() % 10 )
				{
					...
					case 4:
						break;
					case 5:
						//myStats->helmet = newItem(LEATHER_HELM, DECREPIT, -1 + map_rng.rand() % 2, 1, 0, false, NULL);
						break;
					case 6:
					case 7:
					case 8:
					case 9:
						//myStats->helmet = newItem(IRON_HELM, DECREPIT, -1 + map_rng.rand() % 2, 1, 0, false, NULL);
						break;
				}
			}
			...
			if ( myStats->shield == NULL && myStats->EDITOR_ITEMS[ITEM_SLOT_SHIELD] == 1 )
			{
				...
				else
				{
					if ( greaterMonster )
					{
						...
						{
							case 0:
								myStats->shield = newItem(CRYSTAL_SHIELD, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
								break;
							case 1:
								myStats->shield = newItem(STEEL_SHIELD_RESISTANCE, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
								break;
							case 2:
								myStats->shield = newItem(STEEL_SHIELD, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
								break;
							case 3:
								myStats->shield = newItem(MIRROR_SHIELD, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
								break;
							default:
								break;
						}
			...
			if ( myStats->shoes == NULL && myStats->EDITOR_ITEMS[ITEM_SLOT_BOOTS] == 1 )
			{
				if ( greaterMonster )
				{
					switch ( map_rng.rand() % 4 )
					{
						...
						case 2:
						case 3:
							myStats->shoes = newItem(STEEL_BOOTS_LEVITATION, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
							break;
						default:
							break;
					}
			...
			if ( myStats->cloak == NULL && myStats->EDITOR_ITEMS[ITEM_SLOT_CLOAK] == 1 )
			{
				if ( greaterMonster )
				{
					...
					{
						case 0:
							myStats->cloak = newItem(CLOAK_MAGICREFLECTION, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
							break;
						case 1:
							myStats->cloak = newItem(CLOAK_PROTECTION, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
							break;
						case 2:
							myStats->cloak = newItem(CLOAK, EXCELLENT, -1 + map_rng.rand() % 2, 1, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, NULL);
							break;
						case 3:
							break;
						default:
...
void automatonDie(Entity* my)
{
	...
	playSoundEntity(my, 260 + map_rng.rand() % 2, 128);
...
void automatonMoveBodyparts(Entity* my, Stat* myStats, double dist)
{
	...
	if ( ticks % (2 * TICKS_PER_SECOND) == 0 && map_rng.rand() % 5 > 0 )
	{
...
void Entity::automatonRecycleItem()
{
	...
	int pickItem1 = map_rng.rand() % matches; // pick random valid item index in inventory
	int pickItem2 = map_rng.rand() % matches;
	while ( pickItem2 == pickItem1 )
	{
		pickItem2 = map_rng.rand() % matches; // make sure index 2 is unique
	}
	...
	if ( type != GEM_ROCK ) // found an item in category
	{
		...
		if ( map_rng.rand() % 2 == 0 )
		{
			item = newItem(type, item1->status, item1->beatitude, 1, map_rng.rand(), item1->identified, &myStats->inventory);
			item1->status = static_cast<Status>(std::max(0, item1->status - 2));
		}
		else
		{
			item = newItem(type, item2->status, item2->beatitude, 1, map_rng.rand(), item2->identified, &myStats->inventory);
