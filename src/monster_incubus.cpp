	...
	if ( !MONSTER_INIT )
	{
		if ( myStats != nullptr )
		{
			...
			else
			{
				...
				if ( map_rng.rand() % 4 == 0 ) // 1 in 4
				{
					newItem(POTION_CONFUSION, SERVICABLE, 0, 0 + map_rng.rand() % 3, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, &myStats->inventory);
				}
				else // 3 in 4
				{
					newItem(POTION_BOOZE, SERVICABLE, 0, 1 + map_rng.rand() % 3, MONSTER_ITEM_UNDROPPABLE_APPEARANCE, false, &myStats->inventory);
				}
				...
				switch ( defaultItems )
				{
					...
					case 4:
					case 3:
						if ( map_rng.rand() % 2 == 0 && !lesserMonster ) // 1 in 2
						{
							newItem(MAGICSTAFF_COLD, SERVICABLE, 0, 1, map_rng.rand(), false, &myStats->inventory);
						}
					case 2:
						if ( map_rng.rand() % 5 == 0 ) // 1 in 5
						{
							newItem(POTION_CONFUSION, SERVICABLE, 0, 1 + map_rng.rand() % 2, map_rng.rand(), false, &myStats->inventory);
						}
					case 1:
						if ( map_rng.rand() % 3 == 0 ) // 1 in 3
						{
							newItem(POTION_BOOZE, SERVICABLE, 0, 1 + map_rng.rand() % 3, map_rng.rand(), false, &myStats->inventory);
						}
						break;
					default:
						break;
				...
				if ( lesserMonster )
				{
					if ( myStats->weapon == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_WEAPON] == 1 )
					{
						switch ( map_rng.rand() % 10 )
						{
							case 0:
							case 1:
								myStats->weapon = newItem(CROSSBOW, static_cast<Status>(WORN + map_rng.rand() % 2), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
								break;
							case 2:
							case 3:
							case 4:
							...
							case 8:
								...
								break;
							case 9:
								myStats->weapon = newItem(MAGICSTAFF_COLD, static_cast<Status>(WORN + map_rng.rand() % 2), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
								break;
						}
					}
				}
				else if ( myStats->weapon == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_WEAPON] == 1 )
				{
					switch ( map_rng.rand() % 10 )
					{
						...
						case 2:
						case 3:
							myStats->weapon = newItem(CRYSTAL_SPEAR, static_cast<Status>(WORN + map_rng.rand() % 2), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
							break;
						case 4:
						case 5:
						case 6:
				...
				if ( myStats->shield == nullptr && myStats->EDITOR_ITEMS[ITEM_SLOT_SHIELD] == 1 )
				{
					...
					else
					{
						switch ( map_rng.rand() % 10 )
						{
							...
							case 8:
							case 9:
								myStats->shield = newItem(MIRROR_SHIELD, static_cast<Status>(WORN + map_rng.rand() % 2), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, nullptr);
								break;
						}
					}
				}
...
void incubusDie(Entity* my)
{
	...
	if ( myStats && !strncmp(myStats->name, "inner demon", strlen("inner demon")) )
	{
		...
		//playSoundEntity(my, 279 + map_rng.rand() % 3, 128);
				...
				if ( my->monsterAttack > 0 )
				{
						...
						|| my->monsterAttack == MONSTER_POSE_INCUBUS_TAUNT )
					{
						if ( my->monsterAttackTime == 0 )
						{
							...
							{
								playSoundEntityLocal(my, 276 + map_rng.rand() % 3, 128);
							}
							else
							{
	...
	if ( monsterSpecialTimer == 0 && (ticks % 10 == 0) && monsterAttack == 0 )
	{
		...
		if ( trySteal || tryCharm )
		{
			...
			if ( trySteal )
			{
				...
				specialRoll = map_rng.rand() % (40 + 40 * (target->behavior == &actMonster));
			}
			else if ( tryCharm )
			{
				specialRoll = map_rng.rand() % 40;
			}
		...
		// occurs less often against fellow monsters.
		specialRoll = map_rng.rand() % (30 + 30 * (target->behavior == &actMonster));
