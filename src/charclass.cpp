...
void initClass(int player)
{
	...
	if ( client_classes[player] == static_cast<Sint32>(PlayerClass::BARBARIAN) )
	{
		...
		if ( player == clientnum )
		{
			...
			// Booze
			item = newItem(POTION_BOOZE, SERVICABLE, 0, 1, 1, true, nullptr);
			item2 = itemPickup(player, item);
			free(item);
		}
	}
	...
	else if ( client_classes[player] == static_cast<Sint32>(PlayerClass::WANDERER) )
	{
		...
		// Boots
		item = newItem(LEATHER_BOOTS, SERVICABLE, 1, 1, 0, true, nullptr);
		if ( player == clientnum )
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
	...
	else if ( client_classes[player] == static_cast<Sint32>(PlayerClass::CLERIC) )
	{
		...
		// Iron Mace
		item = newItem(IRON_MACE, SERVICABLE, 1, 1, 0, true, nullptr);
		if ( player == clientnum )
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
		...
		// Gloves
		item = newItem(BRACERS, WORN, 1, 1, 0, true, nullptr);
		if ( player == clientnum )
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
	...
	else if ( client_classes[player] == static_cast<Sint32>(PlayerClass::ARCANIST) )
	{
		...
		// Cloak (Red)
		item = newItem(CLOAK, WORN, 0, 1, 2, true, nullptr);
		if ( player == clientnum )
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
		...
		// Hood (Red)
		item = newItem(HAT_HOOD, WORN, 0, 1, 2, true, nullptr);
		if ( player == clientnum )
		{
			item2 = itemPickup(player, item);
			useItem(item2, player);
