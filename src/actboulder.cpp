	...
	if ( entity->behavior == &actPlayer || entity->behavior == &actMonster )
	{
		if ( entityInsideEntity( my, entity ) )
		{
            ...
            if ( stats )
            {
                ...
                // If the Entity survives, or it is a Player wearing an Amulet of Life Saving, destroy the boulder
                if ( stats->HP > 0 || bHasLifeSaving == true )
                {
                    ...
                    if ( !strcmp(map.name, "Sokoban") )
                    {
                        ...
                        if ( monster )
                        {
                            ...
                            for ( c = 0; c < MAXPLAYERS; c++ )
                            {
                                ...
                                messagePlayerColor(c, color, language[406]); // "You have angered the gods of Sokoban!"
                            }
                        }
                    }

                    // Damage the Entity after destroying the boulder to prevent being hit twice
                    entity->modHP(-80);
                    entity->setObituary(language[1505]); // "fails to dodge the incoming boulder."

                    return 1;
                }
                // Damage the Entity after checking if the boulder would be destroyed
                entity->modHP(-80);
                entity->setObituary(language[1505]); // "fails to dodge the incoming boulder."
            }
		}
