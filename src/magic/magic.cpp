	...
	if ( hit.entity )
	{
		...
		if ( hit.entity->behavior == &actMonster || hit.entity->behavior == &actPlayer )
		{
			...
			if ( hitstats->HP > 0 )
			{
				...
				Item* armor = nullptr;
				int armornum = -1;
				if ( hitstats->defending && (local_rng.getU32() % (8 + resistance) == 0) ) // 1 in 8 to corrode shield
				{
					armornum = hitstats->pickRandomEquippedItem(&armor, true, false, true, true);
				}
				else if ( !hitstats->defending && (local_rng.getU32() % (4 + resistance) == 0) ) // 1 in 4 to corrode armor
				{
	...
	if ( hit.entity )
	{
		if ( hit.entity->behavior == &actMonster || hit.entity->behavior == &actPlayer )
		{
			...
			if ( hit.entity->setEffect(EFF_WEBBED, true, 400, true) ) // 8 seconds.
			{
				...
				{
					playSoundEntity(hit.entity, 396 + local_rng.getU32() % 3, 64); // play sound only if not recently webbed. (triple shot makes many noise)
				}
...
spell_t* spellEffectVampiricAura(Entity* caster, spell_t* spell, int extramagic_to_use)
{
	...
	//	{
	//		duration -= local_rng.getU32() % (1000 / (spellcastingPower + 1)); // reduce the duration by 0-20 seconds
	//	}
	//	if ( duration < 50 )
	//	{
...
void spellEffectCharmMonster(Entity& my, spellElement_t& element, Entity* parent, int resistance, bool magicstaff)
{
	if ( hit.entity )
	{
		if ( hit.entity->behavior == &actMonster || hit.entity->behavior == &actPlayer )
		{
			...
			else if ( parent && local_rng.getU32() % 100 < chance
...
Entity* spellEffectPolymorph(Entity* target, Entity* parent, bool fromMagicSpell, int customDuration)
{
	...
	effectDuration = TICKS_PER_SECOND * 60 * (4 + local_rng.getU32() % 3); // 4-6 minutes
	if ( customDuration > 0 )
	{
	...
	if ( target->behavior == &actMonster )
	{
		...
		else
		{
			...
			monsterSummonType = possibleTypes.at(local_rng.getU32() % possibleTypes.size());
		}
	...
	else if ( target->behavior == &actPlayer )
	{
		if ( target->setEffect(EFF_POLYMORPH, true, effectDuration, true) )
		{
			...
			{
				int roll = (RACE_HUMAN + 1) + local_rng.getU32() % 8;
				...
				else
				{
					...
					{
						roll = (RACE_HUMAN + 1) + local_rng.getU32() % 8; // re roll to not polymorph into the same thing
					}
					target->effectPolymorph = target->getMonsterFromPlayerRace(roll);
				}
			}
			...
			{
				target->effectPolymorph = 100 + local_rng.getU32() % NUMAPPEARANCES;
			}
			...
			/*|| target->behavior == &actDoor || target->behavior == &actChest*/ )
		{
			...
			if ( parent )
			{
				...
				if ( !foundLocation )
				{
					...
					{
						std::pair<int, int> tmpPair = spotsWithLineOfSight[local_rng.getU32() % spotsWithLineOfSight.size()];
						tx = tmpPair.first;
						ty = tmpPair.second;
					}
					else if ( !goodspots.empty() )
					{
						std::pair<int, int> tmpPair = goodspots[local_rng.getU32() % goodspots.size()];
						tx = tmpPair.first;
						ty = tmpPair.second;
					}
					else
