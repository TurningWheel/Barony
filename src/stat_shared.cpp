	...
	monsterForceAllegiance(MISC_FLAGS[20])
{
	...
	this->sex = static_cast<sex_t>(local_rng.getU32() % 2);
	this->appearance = 0;
	this->HP = 10;
	this->MAXHP = 10;
	this->OLDHP = this->HP;
...
void setDefaultMonsterStats(Stat* stats, int sprite)
{
	switch ( sprite )
	{
		...
		case (1000 + GNOME):
			stats->type = GNOME;
			stats->sex = static_cast<sex_t>(local_rng.getU32() % 2);
			stats->appearance = 0;
			stats->HP = 50;
			stats->MAXHP = 50;
			stats->MP = 50;
		...
		case (1000 + DEVIL):
			stats->type = DEVIL;
			stats->sex = static_cast<sex_t>(local_rng.getU32() % 2);
			stats->appearance = local_rng.getU32();
		...
		case (1000 + HUMAN):
			stats->type = HUMAN;
			stats->sex = static_cast<sex_t>(local_rng.getU32() % 2);
			stats->appearance = local_rng.getU32() % 18; //NUMAPPEARANCES = 18
			stats->inventory.first = NULL;
			stats->inventory.last = NULL;
			stats->HP = 30;
			stats->RANDOM_HP = 20;
