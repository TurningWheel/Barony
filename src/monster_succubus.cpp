	...
	if ( !MONSTER_INIT )
	{
		if ( myStats != nullptr )
		{
			...
			switch ( defaultItems )
			{
				...
				case 2:
				case 1:
					if ( !strcmp(myStats->name, "Lilith") && map_rng.rand() % 4 > 0 )
					{
						newItem(MAGICSTAFF_CHARM, EXCELLENT, -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, &myStats->inventory); // 75% chance
					}
					else if ( map_rng.rand() % 10 == 0 )
					{
						newItem(MAGICSTAFF_CHARM, static_cast<Status>(DECREPIT + map_rng.rand() % 2), -1 + map_rng.rand() % 3, 1, map_rng.rand(), false, &myStats->inventory); // 10% chance
					}
					break;
				default:
					break;
